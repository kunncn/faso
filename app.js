...
//                 <span>${item.name} ${itemTA} x${item.qty}</span>
...
function confirmAddToCart() {
  ...
  const finalPrice = selectedTemp === "ice"
    ? selectedProduct.priceIce
...
function processSale() {
  ...
  const sale = {
    ...
    items: structuredClone(cart).map((item) => ({
      ...
      category: item.category,
      addons: item.addons || [],
      addonTotal: item.addons ? item.addons.reduce((s, a) => s + a.price, 0) : 0,
      lineTotal: (item.price + (item.addons ? item.addons.reduce((s, a) => s + a.price, 0) : 0)) * item.qty,
    })),
    payment: paymentMethod,
    orderType,
    subtotal: sub,
...
function exportPDF() {
  getTodaySales((sales) => {
    ...
    getTodaySummary(({ cash, card, qr, items }) => {
      ...
      doc.text("FASO DAILY REPORT", 10, y); y += 10;
      doc.text(`Cash: RM${cash.toFixed(2)}`, 10, y); y += 8;
      doc.text(`Card: RM${card.toFixed(2)}`, 10, y); y += 8;
      doc.text(`QR: RM${qr.toFixed(2)}`, 10, y); y += 10;
      doc.text(`TOTAL: RM${total.toFixed(2)}`, 10, y); y += 12;
      doc.text("ITEMS SOLD:", 10, y); y += 10;
...
function renderOrderHistory() {
  getTodaySales((sales) => {
    ...
    if (sales.length === 0) {
      container.innerHTML = '<p class="text-sm text-gray-400">No orders today</p>';
      return;
    }
...
function showOrderHistory() {
  getTodaySales((sales) => {
    ...
    if (sales.length === 0) {
      history.innerHTML = '<p class="text-gray-400 text-center py-6">No orders today</p>';
      document.getElementById("history-modal").classList.remove("hidden");
      return;
    }
    ...
    const totalRevenue = sales.reduce((s, sale) => s + sale.total, 0);
    const totalOrders = sales.length;
    const cashTotal = sales.filter(s => s.payment === "cash").reduce((s, sale) => s + sale.total, 0);
    const cardTotal = sales.filter(s => s.payment === "card").reduce((s, sale) => s + sale.total, 0);
    const qrTotal = sales.filter(s => s.payment === "qr").reduce((s, sale) => s + sale.total, 0);
    ...
    const ordersHTML = sales
      ...
      .reverse()
      .map((sale, idx) => {
        const discountLabel = sale.discountRate > 0
          ? `<span class="text-xs bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded">${(sale.discountRate * 100).toFixed(0)}% OFF</span>`
          : "";

        const taLabel = sale.orderType === "takeaway"
          ? `<span class="text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded">TA</span>`
          : `<span class="text-xs bg-green-100 text-green-600 px-2 py-0.5 rounded">Dine In</span>`;
        ...
        const itemsHTML = sale.items.map((item) => {
          ...
          const itemTA = item.orderType === "takeaway"
            ? `<span class="text-[10px] bg-red-100 text-red-500 px-1 rounded">TA</span>`
            : "";

          const itemTemp = item.temp === "hot"
