...
function renderBestSellers() {
  getAllSales((allSales) => {
    ...
    const itemsArr = Object.keys(itemStats).map((name) => ({
      name,
      category: itemStats[name].category,
      qty: itemStats[name].qty,
      revenue: itemStats[name].revenue,
    }));

    const container = document.getElementById("best-sellers-body");
    ...
    const buildRows = (list, valueKey, formatFn) =>
      list
        ...
        <tr class="border-b border-gray-100">
          <td class="py-2 pr-2 text-sm text-gray-400 w-8">${idx + 1}</td>
          <td class="py-2 pr-2 text-sm text-gray-800">
            <span class="mr-1">${categoryEmoji[item.category] || "🍽️"}</span>${item.name}
          </td>
          <td class="py-2 text-sm text-right font-semibold text-[#008697]">${formatFn(item[valueKey])}</td>
        </tr>
      `,
        )
        .join("");

    const buildTable = (list, valueKey, valueLabel, formatFn) => `
      <table class="w-full border-collapse">
        <thead>
          <tr class="text-xs text-gray-400 uppercase tracking-wide">
            <th class="text-left font-medium py-2 w-8">#</th>
            <th class="text-left font-medium py-2">Item</th>
            <th class="text-right font-medium py-2">${valueLabel}</th>
          </tr>
        </thead>
        <tbody>${buildRows(list, valueKey, formatFn)}</tbody>
      </table>
    `;
